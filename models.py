...
# -*- coding: utf-8 -*-
"""
Shared models and utilities for the CIVENG794 project.

Includes:
- Wiedemann-99 style thresholds and regime selection (implemented form used in this project)
- Acceleration rules per regime
- A simple integration scheme for trajectory simulation
- Utility functions (RMSE, sigmoid, log loss)

Notes on methodology consistency:
- The CF calibration objective (in calibrate_cf.py) uses gap + SPEED RMSE (not Δv RMSE).
- The MOBIL LC model (in calibrate_lc.py) is probabilistic (logistic choice) and does not use
  a hard acceleration threshold a_thr (removed for consistency).
"""
...
class CCParams:
    """
    Wiedemann-99 parameter container.

    In this project:
      - CC0, CC1, CC2 are calibrated (standstill, headway time, additional gap)
      - CC3..CC8 are held fixed to defaults
      - CC9 is not used in this simplified implementation (kept for completeness)
    """
    CC0: float
    CC1: float
    CC2: float
    ...
    CC7: float
    CC8: float
    CC9: Optional[float] = None


@dataclass(frozen=True)
class VehicleParams:
    """
    Generic vehicle behavior / physical limits for the simulation.
    Units:
      - v_free: ft/s
      - bmin:  ft/s^2 (negative number, maximum comfortable/allowed braking)
    """
    v_free: float
    bmin: float
    tau: float = 1.0
...
# ============================================================
# Wiedemann thresholds (implemented form used consistently across scripts)
# ============================================================

def thresholds_w99(cc: CCParams, DX: float, Vlead: float) -> Dict[str, float]:
    """
    Implemented threshold equations (as used in the code base).

    DX    = current net gap (ft)
    Vlead = leader speed (ft/s)

    Returns: dict with AX, ABX, SDX, CLDV, OPDV, SDV
    """
    AX = cc.CC0
    ABX = AX + cc.CC1 * Vlead
    SDX = ABX + cc.CC2

    # Implemented speed thresholds:
...
def classify_regime(DX: float, DV: float, th: Dict[str, float]) -> Regime:
    """
    DV is defined as follower speed minus leader speed:
      DV = vF - vL

    Regimes follow the rule structure used in the project.
    """
...
def bmax_free_flow(cc: CCParams, veh: VehicleParams, v_f: float, DX: float, ABX: float) -> float:
    """
    Free-flow desired acceleration (implemented form).
    """
    if DX <= ABX:
        return 0.0
...
def accel_closing(cc: CCParams, veh: VehicleParams, DV: float, DX: float) -> float:
    """
    Closing regime braking rule (implemented form).
    """
...
def accel_following(cc: CCParams, Bmax: float, DV: float) -> float:
    """
    Following regime rule (simple oscillation).
    """
    if DV < 0:
...
def accel_emergency(cc: CCParams, veh: VehicleParams, DV: float, DX: float, ABX: float, b_prev: float) -> float:
    """
    Emergency braking rule (implemented form with strong safety behavior).
    """
    if DX <= cc.CC0:
        return veh.bmin
...
def simulate_episode_wiedemann(
    t: np.ndarray,
    xL: np.ndarray,
    vL: np.ndarray,
    L_lead: np.ndarray,
    ...
    """
    Simulate a single leader trajectory and generate follower speed, gap, and accel arrays.

    Returns:
      vF (ft/s), gap (ft), bF (ft/s^2)
    """
    n = len(t)
    if n < 2:
    ...
    for k in range(n - 1):
        DX = float(max((xL[k] - xF[k]) - L_lead[k], 0.0))
        DV = float(vF[k] - vL[k])
        ...
        # Safety guard: do not pass leader's rear bumper
        max_allowed_pos = float(xL[k + 1] - L_lead[k + 1] - 0.1)
        if x_next >= max_allowed_pos:
            xF[k + 1] = max_allowed_pos
            vF[k + 1] = min(float(v_next), float(vL[k + 1]))
            b_next = veh.bmin
        else:
            xF[k + 1] = float(x_next)
            vF[k + 1] = max(0.0, float(v_next))
...
def accel_from_state(cc: CCParams, veh: VehicleParams, gap: float, vF: float, vL: float, L_lead: float) -> float:
    """
    Convenience: compute instantaneous acceleration from (gap, vF, vL).
    Used for MOBIL incentives and stability analysis.
    """
