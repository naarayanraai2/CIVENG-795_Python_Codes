...
# -*- coding: utf-8 -*-
"""
Car-following calibration (Wiedemann-99, implemented form).

Correct methodology:
- Uses bounded random search (Monte Carlo sampling) over CC0–CC2 ranges.
- Objective minimizes GAP RMSE + SPEED RMSE over selected steady-following episodes.
  (Not Δv RMSE; consistent with the code and corrected report.)
"""
...
from models import CCParams, VehicleParams, rmse, simulate_episode_wiedemann
...
class EpisodeArrays:
    ...
    v_obs: np.ndarray
    gap_obs: np.ndarray
    meta: Tuple[int, int, int]  # (follower_id, leader_id, lane)


def load_ngsim(csv_path: str) -> pd.DataFrame:
    df = pd.read_csv(csv_path)
    needed = [
        "Vehicle_ID", "Frame_ID", "Local_Y", "Lane_ID",
        "v_Vel", "v_Acc", "Preceding", "Space_Headway",
        "v_Length", "v_Class"
    ]
    ...
    # Passenger cars only (NGSIM class=2)
...
def build_cf_with_leader(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cf = cf[cf["Lane_ID"] == cf["Leader_Lane"]].copy()  # same-lane leader-following
...
def stratified_select_episodes(
    cf: pd.DataFrame,
    fps: float,
    n_total: int = 50,
    ...
    """
    Keeps your original “stratified flavor” to get diverse episodes:
    - short gaps / low speed
    - stable following
    - more oscillatory following
    """
...
def precompute_episode_arrays(episodes: List[pd.DataFrame], fps: float) -> List[EpisodeArrays]:
    ...
    for ep in episodes:
        ...
        out.append(EpisodeArrays(
            ...
            v_obs=v_obs, gap_obs=gap_obs,
            meta=meta
        ))
    return out


def objective_cc0_cc1_cc2(
    theta: np.ndarray,
    ...
    """
    Correct objective (consistent with code and corrected report):
      J = w_gap * mean(RMSE_gap)/gap_scale + w_v * mean(RMSE_speed)/v_scale
    """
    CC0, CC1, CC2 = map(float, theta)

    cc = CCParams(
        CC0=CC0, CC1=CC1, CC2=CC2,
    ...
    gap_errs, v_errs = [], []
    for ep in episodes:
        v_sim, gap_sim, _ = simulate_episode_wiedemann(
...
def random_search(bounds: np.ndarray, n_samples: int, seed: int, obj_kwargs: dict):
    """
    Bounded random search (Monte Carlo parameter sampling).
    """
    rng = np.random.default_rng(seed)
    best_val = np.inf
    best_theta = None
...
def plot_best_fit(episodes: List[EpisodeArrays], cc_best: CCParams, veh: VehicleParams, n_show: int = 4, out_png: str | None = None):
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--fps", type=float, default=10.0, help="NGSIM frame rate (Hz)")
    ap.add_argument("--episodes", type=int, default=50, help="Number of episodes to sample")
    ap.add_argument("--min-seconds", type=float, default=8.0, help="Minimum episode duration")
    ap.add_argument("--samples", type=int, default=800, help="Random search samples")
    ap.add_argument("--seed", type=int, default=42, help="RNG seed")
    ap.add_argument("--out", default="cf_best_params.csv", help="Output CSV for best params")
    ap.add_argument("--plot", default="cf_fit.png", help="Output plot PNG (best-fit episodes)")
...
def main():
    ...
    print("Loading + building CF table...")
    df = load_ngsim(args.csv)
    cf = build_cf_with_leader(df)

    print("Selecting episodes...")
    eps_df = stratified_select_episodes(cf, fps=args.fps, n_total=args.episodes, min_seconds=args.min_seconds, seed=args.seed)
    eps = precompute_episode_arrays(eps_df, fps=args.fps)
    print(f"Episodes: {len(eps)}")

    # Search bounds: [CC0, CC1, CC2]
    bounds = np.array([
        [3.0, 10.0],
        [0.3, 3.0],
        [6.0, 40.0],
    ...
    best_theta, best_obj = random_search(
        ...
        seed=args.seed,
        obj_kwargs=dict(
            template_cc=template_cc, veh=veh, episodes=eps,
    ...
    plot_best_fit(eps, cc_best, veh, n_show=4, out_png=args.plot)
